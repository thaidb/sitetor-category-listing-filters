...
// Thanh filter đơn giản (bản gốc) — giữ nguyên cho cây category Mapping
// (nhãn Ngân sách). Giá trị đẩy vào query param của discovery route; backend
// plugin sitetor-listing (TopicQuery custom filters) lọc server-side.
const PRICE_UNITS = { million: 1e6, billion: 1e9 };

export default class MappingFiltersLegacy extends Component {
  @service router;
  ...
  <template>
    <div class="sitetor-cat-filters">
      <select class="scf-select" {{on "change" (fn this.setSelect "type")}}>
      ...
      <span class="scf-group">
        <label>{{i18n (themePrefix "scf.ngan_sach")}}</label>
